<template>
  <footer class="bg-[var(--md-sys-color-background)]">
    <div class="px-4 py-12 mx-auto max-w-7xl sm:px-6 lg:px-8 border-t border-[var(--md-sys-color-outline-variant)]">
      <div class="md:flex md:items-center md:justify-between">
        <div class="flex justify-center space-x-6 md:order-2">
          <a class="footer-icon" href="https://discord.gg/U9fZSJJcte" target="_blank">
            <span class="sr-only">Discord</span>
            <i class="fa-brands fa-discord"></i>
          </a>
          <a class="footer-icon" href="https://www.youtube.com/@GensokyoReimagined" target="_blank">
            <span class="sr-only">youtube</span>
            <i class="fa-brands fa-youtube"></i>
          </a>
          <a class="footer-icon" href="https://www.buymeacoffee.com/greimagined/membership" target="_blank">
            <span class="sr-only">BuyMeACoffee</span>
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
              <path
                  d="M294.314667 0L242.432 119.424H165.418667v107.648h29.653333L225.152 418.133333H178.005333l62.293334 351.146667 40.021333-0.426667 40.192 255.146667h380.501333l2.645334-17.066667 37.546666-238.08 37.888 0.426667 62.293334-351.189333h-45.056l30.08-191.018667h32.256V119.466667h-81.834667L724.906667 0H294.314667z m22.528 34.346667h385.834666l32.896 75.946666H283.818667l33.024-75.946666z m-117.333334 119.338666H824.32v39.253334H199.509333v-39.253334z m19.328 298.581334h581.76l-50.176 282.453333-241.024-2.56-240.469333 2.56-50.090667-282.453333z"/>
            </svg>
          </a>
          <a class="footer-icon" href="https://afdian.com/a/gensokyoreimagined?tab=home" target="_blank">
            <span class="sr-only">Afdian</span>
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
              <path
                  d="M383.2 607.072a24.576 24.576 0 1 0 24.576 24.576 24.576 24.576 0 0 0-24.576-24.576z m228.448 70.848a24.576 24.576 0 1 0 0 49.12 24.576 24.576 0 0 0 0-49.12z"/>
              <path
                  d="M966.72 738.912c-8.064 4.032-19.616 4.224-30.272 0.544-7.2-2.528-14.976-4.928-22.72-7.328-14.368-4.384-38.4-11.712-42.944-17.248a22.656 22.656 0 1 0-36.224 27.296c2.496 3.392 5.888 6.176 9.312 8.864-34.656 47.968-186.88 143.456-402.112 96.416-96.992-21.216-162.272-59.936-188.832-111.936-19.136-37.568-14.784-76.256-5.632-97.28 26.336-54.72 216.544-180.768 337.984-252.128a22.72 22.72 0 0 0 8.032-31.072c-6.368-10.784-20.832-15.104-31.04-8.032-9.792 5.76-43.104 25.536-85.6 52.448-14.368-9.664-39.2-21.568-79.008-25.472-63.136-6.272-209.024-34.112-256.16-54.048-33.76-14.272-110.4-52.096-93.408-94.656 3.936-9.92 22.4-25.984 62.464-41.28a357.184 357.184 0 0 1 84.768-20.864l-8.992 4.352c-27.296 13.248-57.056 28.064-77.056 45.6-1.44-0.928-2.976-1.888-4-2.912a22.4 22.4 0 0 0-32.032-0.736 22.624 22.624 0 0 0-0.736 32.064c8.064 8.48 20.032 14.848 35.232 19.712 0.576 0.192 1.088 0.448 1.664 0.576 52.896 16.288 146.912 14.24 279.456 8.992 57.152-2.24 111.104-4.352 156.128-3.488 149.216 2.976 261.376 62.304 342.88 181.472 50.08 73.216 32 145.344 3.36 208a60.896 60.896 0 0 1-9.824-9.44 22.976 22.976 0 0 0-31.84-3.616 22.688 22.688 0 0 0-3.616 31.872c4.608 5.824 28.256 32.992 57.568 30.688 0.544 0.032 1.024-0.032 1.568-0.064 0.864-0.128 1.792 0.032 2.72-0.128 20.032-3.648 32.608-2.688 44.384 4.672 15.584 9.824 23.264 15.712 21.728 20.608-0.896 3.072-3.232 5.28-7.136 7.488zM348 486.176L334.624 496a67.2 67.2 0 0 1-10.336-13.6c5.28-1.568 14.048 0.288 23.712 3.776z m625.984 184.8a79.104 79.104 0 0 0-34.432-11.456c26.88-65.504 43.968-151.232-14.24-236.384-89.056-130.304-216.768-197.984-379.456-201.248-46.336-0.896-100.992 1.248-158.784 3.552-67.136 2.656-156.448 6.144-217.184 1.12 12.096-6.56 24.992-12.864 36.16-18.272 42.72-20.768 76.352-36.96 65.216-65.664-5.824-16.768-23.232-25.568-51.808-26.208-60.16-1.376-190.208 33.44-213.568 92.096-13.152 33.024-15.328 96.8 117.92 153.152 54.816 23.232 209.632 51.552 269.376 57.472 16.896 1.632 29.792 5.056 39.616 8.672a2839.468 2839.468 0 0 0-41.888 28.16c-24.544-13.984-64.64-30.464-94.592-10.048-11.584 7.84-18.176 19.392-18.496 32.544-0.416 16.352 9.504 32.352 20.672 45.248-44.096 35.168-79.776 69.28-92.416 95.552-14.112 32.32-19.84 85.184 6.464 136.768 33.408 65.472 107.232 111.104 219.552 135.68 146.752 32.032 273.504 3.168 357.376-40.96 46.688-24.576 79.808-53.888 95.904-80.032l15.136 4.768c7.168 2.176 14.336 4.352 21.056 6.752 22.208 7.68 46.784 6.656 65.664-2.88l1.056-0.544c14.24-7.488 24.512-19.552 29.024-34.048 12.096-39.136-22.432-60.768-43.264-73.824z"/>
            </svg>
          </a>
          <a class="footer-icon" href="https://ko-fi.com/gensokyoreimagined" target="_blank">
            <span class="sr-only">Kofi</span>
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
              <path
                  d="M1018.922667 381.781333c-32.981333-174.293333-207.317333-195.968-207.317334-195.968H30.848c-25.770667 0-28.970667 34.048-28.970667 34.048s-3.498667 312.490667-0.938666 504.405334c6.997333 103.424 110.336 114.005333 110.336 114.005333s352.725333-0.981333 510.549333-2.090667c104.021333-18.176 114.474667-109.482667 113.408-159.317333 185.685333 10.24 316.672-120.789333 283.690667-295.082667z m-471.978667 149.802667c-53.162667 61.994667-171.136 169.642667-171.136 169.642667s-5.162667 5.077333-13.226667 0.981333c-3.242667-2.432-4.608-3.84-4.608-3.84-18.901333-18.816-143.701333-130.090667-172.117333-168.704-30.250667-41.173333-44.416-115.2-3.882667-158.293333 40.576-43.093333 128.213333-46.336 186.154667 17.365333 0 0 66.773333-76.032 147.968-41.088 81.237333 34.986667 78.165333 128.469333 30.848 183.936z m263.381333 20.394667c-39.594667 4.949333-71.765333 1.194667-71.765333 1.194666V310.784h75.52s84.096 23.509333 84.096 112.554667c0 81.621333-42.026667 113.792-87.850667 128.64z"/>
            </svg>
          </a>
        </div>
        <div class="mt-8 md:mt-0 md:order-1">
          <p class="text-base text-center text-gray-400">
            &copy; 2022-2026 Gensokyo-Reimagined —
            <a class="hover:text-[var(--md-sys-color-primary)] transition-colors"
               href="https://github.com/Gensokyo-Reimagined"
               target="_blank">@Gensokyo-Reimagined</a>
          </p>
          <p class="mt-2 text-sm text-center text-gray-500">
            Payments securely handled by
            <a class="hover:text-[var(--md-sys-color-primary)] transition-colors"
               href="https://paynow.gg" target="_blank">PayNow</a>
            &middot;
            <a class="hover:text-[var(--md-sys-color-primary)] transition-colors"
               href="https://paynow.gg/legal/terms-of-use" target="_blank">Terms of Service</a>
            &middot;
            <a class="hover:text-[var(--md-sys-color-primary)] transition-colors"
               href="https://paynow.gg/legal/privacy-policy" target="_blank">Privacy Policy</a>
          </p>
        </div>
      </div>
    </div>
  </footer>
</template>

<style scoped>
.footer-icon {
  @apply text-gray-400 hover:text-[var(--md-sys-color-primary)] transition-all duration-300 transform hover:scale-125;
}

.footer-icon svg {
  @apply w-6 h-6;
}
</style>
<script lang="ts" setup>
</script>