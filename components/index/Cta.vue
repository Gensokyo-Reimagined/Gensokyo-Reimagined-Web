...
.animate-fog-drift-slow { animation: fog-drift-slow 20s ease-in-out infinite; }
.bg-gradient-radial { background: radial-gradient(circle at 30% 50%, var(--tw-gradient-stops)); }
.bg-gradient-radial-offset { background: radial-gradient(circle at 70% 60%, var(--tw-gradient-stops)); }
@keyframes shimmer-slow { 0% { transform: translateX(-100%) translateY(-100%) rotate(30deg); } 100% { transform: translateX(100%) translateY(100%) rotate(30deg); } }
.animate-shimmer-slow { animation: shimmer-slow 15s ease-in-out infinite; }
@keyframes pulse-subtle { 0%, 100% { opacity: 0.2; } 50% { opacity: 0.4; } }
.animate-pulse-subtle { animation: pulse-subtle 5s ease-in-out infinite; }
.image-tilt-container { position: relative; border-radius: 1rem; transform-style: preserve-3d; cursor: pointer; }
.image-itself { width: 100%; height: 100%; border-radius: inherit; border: 4px solid rgba(255, 255, 255, 0.3); box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); transition: border-color 0.5s ease; position: relative; z-index: 1; }
.image-tilt-container:hover .image-itself { border-color: rgba(255, 255, 255, 0.5); }
.image-tilt-container::before { content: ''; position: absolute; inset: 0; background: linear-gradient(to top, rgba(255, 255, 255, 0.15), transparent); border-radius: inherit; z-index: 2; pointer-events: none; }
.image-tilt-container::after { content: ''; position: absolute; inset: -2rem; background: radial-gradient(circle, rgba(255, 255, 255, 0.05), transparent); filter: blur(32px); animation: pulse-subtle 5s ease-in-out infinite; z-index: 0; }
.drop-shadow-lg { filter: drop-shadow(0 10px 20px rgba(0, 0, 0, 0.3)); }
